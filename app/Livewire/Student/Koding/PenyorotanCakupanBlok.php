...
class PenyorotanCakupanBlok extends Component
{
    ...
    public function ketuk(int $index)
    {
        $this->barisAktif = $index;
        $this->sudahDiketuk[$index] = true;
    }
    public function toggleLekukan()
    {
        $this->tampilkanLekukan = ! $this->tampilkanLekukan;
    }
    ...
    public function getSemuaSudahDiketukProperty(): bool
    {
        return count($this->sudahDiketuk) >= count($this->konfigurasi['baris'] ?? []);
    }
    ...
    {
        return view('livewire.student.koding.penyorotan-cakupan-blok', [])->layout('layouts.questify');
    }
}
