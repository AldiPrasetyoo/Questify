...
class EvaluatorKondisi extends Component
{
    ...
    public function updatedJam() { $this->sudahDicoba = true; }

    private function evaluasi(string $operator, int $pembanding): bool
    {
        return match ($operator) {
            '==' => $this->jam === $pembanding, '!=' => $this->jam !== $pembanding,
            '>' => $this->jam > $pembanding, '<' => $this->jam < $pembanding,
            '>=' => $this->jam >= $pembanding, '<=' => $this->jam <= $pembanding,
            default => false,
        };
    ...
    public function render() { return view('livewire.student.koding.evaluator-kondisi'); }
}
