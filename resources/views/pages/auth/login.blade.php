...
<!-- Menggunakan min-h-screen dan overflow-y-auto sebagai jaring pengaman agar tidak terpotong di layar super kecil -->

<body
    class="min-h-screen w-screen bg-[#e0f7fa] relative overflow-x-hidden overflow-y-auto flex flex-col justify-between">

    <!-- Navbar: Ukuran logo dan teks dikembalikan sedikit lebih besar -->
    ...
    <main class="flex flex-col justify-center items-center relative z-30 px-4 w-full max-w-6xl mx-auto mt-[-5px]">
        ...
        <!-- Judul: Ukuran font sedikit dinaikkan -->
        ...
        <div class="w-full flex flex-col md:flex-row justify-center items-center gap-4 relative z-30">
            ...
            <!-- Area Kiri: Robot sedikit diperbesar -->
            ...
            <!-- Area Tengah: Kartu Login diperlebar sedikit -->
            <div
                ...
                <div class="p-5 md:px-6 md:py-5">
                    ...
                    <div class="text-center mb-3">
                        ...
                        <h2 class="text-xs font-extrabold text-gray-800 tracking-wide">SELAMAT DATANG!</h2>
                    </div>

                    <x-auth-session-status class="mb-3 text-center text-[11px] text-green-600 font-bold"
                        :status="session('status')" />

                    <!-- Passkey: Ukuran tombol disesuaikan menjadi teks-xs dan padding proporsional -->
                    <div
                        class="mb-3 [&>div>button]:!bg-[#1f2937] [&>div>button]:!text-white [&>div>button]:!rounded-full [&>div>button]:!text-xs [&>div>button]:!py-2 [&>div]:!mb-2 [&>div>div]:!my-2">
                        <x-passkey-verify />
                    </div>
                    ...
                    <form method="POST" action="{{ route('login.store') }}">
                        ...
                        <div class="mb-3">
                            <input id="email" type="email" name="email" value="{{ old('email') }}"
                                placeholder="Username/Email" required autofocus autocomplete="username"
                        ...
                        <div class="flex items-center justify-between mb-4 px-1">
                            <label for="remember_me" class="inline-flex items-center cursor-pointer">
                                <input id="remember_me" type="checkbox" name="remember"
                                    class="rounded border-gray-300 text-[#ff8a00] shadow-sm focus:ring-[#ff8a00] w-3 h-3"
                                    {{ old('remember') ? 'checked' : '' }}>
                                <span class="ms-2 text-[10px] font-semibold text-gray-500">{{ __('Ingat saya') }}</span>
                            </label>

                            @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" wire:navigate
                                class="text-[10px] font-bold text-gray-400 hover:text-[#00c2cb] transition-colors">
                                Lupa Password?
                            </a>
                            @endif
                        </div>
                        ...
                        <p class="text-center text-[11px] font-semibold text-gray-500">
                            Belum punya akun? <a href="{{ route('register') }}" wire:navigate
                                class="text-[#00c2cb] font-extrabold hover:underline">Daftar Sekarang</a>
                        </p>
                    </form>
                </div>
            </div>
