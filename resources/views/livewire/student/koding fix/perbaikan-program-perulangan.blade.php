...
12          cout << \" Jam siswa ke-\" << i << \": \"; 13 cin>> jam;
    ...
    16 for (int k = 0; k <= jumlah; k++) { 17 cout << \"Data ke-\" << k << \" tercatat\" << endl; 18 } 19 20 int m=1; 21
        do { 22 cout << \"Rekap baris \" << m << endl; 23 m++; 24 } while (m <=jumlah);`, kodePerbaikan: `int i=1; while
        (i <=jumlah) { cout << \"Jam siswa ke-\" << i << \": \"; cin>> jam;
        i++;
        }

        for (int k = 1; k <= jumlah; k++) { cout << \"Data ke-\" << k << \" tercatat\" << endl; } int m=1; while (m
            <=jumlah) { cout << \"Rekap baris \" << m << endl; m++; }`, dataUji: [{jumlah: 3, catatan: '' }, {jumlah: 0,
            catatan: 'Data uji penentu untuk kesalahan do-while' }], hasilUji: [], errorTerakhir: null, sudahDijalankan:
            false, sedangJalan: false, async jalankanUji() { this.sedangJalan=true; this.hasilUji=[];
            this.errorTerakhir=null; await new Promise(r=> setTimeout(r, 10));
            ...
            const program = `#include <iostream>
                ...
                if (hasil.error) { this.errorTerakhir = hasil.error; this.hasilUji = []; this.sudahDijalankan = true;
                this.sedangJalan = false; return; }
                ...
                this.hasilUji.push({
                jumlah: d.jumlah, catatan: d.catatan, cinAktual, cinSeharusnya, barisAktual, barisSeharusnya,
                batasWaktu: hasil.batasWaktuTerlampaui,
                lolos: !hasil.batasWaktuTerlampaui && cinAktual === cinSeharusnya && barisAktual === barisSeharusnya,
                });
                }
                ...
                <p class="text-sm font-semibold text-gray-600 mb-2">Program berikut berisi tiga perulangan dengan empat
                    kesalahan tersebar di dalamnya.</p>
                ...
                <details class="mb-4 text-sm">
                    <summary class="cursor-pointer text-[#00c2cb] font-bold">Bantuan: telusuri manual dulu untuk masukan
                        3 dan 0</summary>
                    <p class="text-gray-500 font-semibold mt-2">Perhatikan baris 10-11, lalu periksa apakah nilai i
                        pernah bertambah. Terakhir, kalau jumlah = 0, haruskah blok baris 21-24 tetap berjalan?</p>
                </details>
                                ...
                                <template x-for="h in hasilUji" :key="h.jumlah">
                                    <tr class="border-t" :class="h.lolos ? '' : 'bg-rose-50'">
                                        <td class="p-2 font-mono">
                                            ...
                                            <template x-if="h.catatan"><span
                                                    class="text-xs text-amber-600 font-semibold block"
