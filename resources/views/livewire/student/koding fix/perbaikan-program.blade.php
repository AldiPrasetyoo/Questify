...
17          cout << \" - harap menghadap guru piket\";`,
    ...
    dataUji: [[648,'Hadir'],[715,'Terlambat'],[800,'Alpa']],
    hasilUji: [],
    errorTerakhir: null,
    ...
    async jalankanUji() {
        ...
        for (const [jam, seharusnya] of this.dataUji) {
            const program = window.bungkusKode(this.kodePerbaikan, { jam });
            ...
            this.hasilUji.push({ jam, seharusnya, keluaran: hasil.keluaran, lolos: hasil.keluaran.trim() === seharusnya });
        }
    ...
    <p class="text-sm font-semibold text-gray-600 mb-2">Program berikut disisipi empat kesalahan. Temukan dan perbaiki di editor di bawah.</p>
    <div class="font-mono bg-slate-900 text-rose-300 text-sm rounded-2xl p-4 mb-4 whitespace-pre" x-text="kodeBermasalah"></div>

    <details class="mb-4 text-sm">
        <summary class="cursor-pointer text-[#00c2cb] font-bold">Bantuan: telusuri manual dulu untuk jam 648, 715, dan 800</summary>
        <p class="text-gray-500 font-semibold mt-2">Coba jalankan program di atas di kepalamu untuk ketiga nilai jam tersebut. Bandingkan hasilnya dengan yang seharusnya: 648 &rarr; Hadir, 715 &rarr; Terlambat, 800 &rarr; Alpa.</p>
    </details>

    <p class="text-sm font-bold text-gray-700 mb-1">Tulis versi perbaikanmu:</p>
    <textarea x-model="kodePerbaikan" rows="7" class="w-full font-mono text-sm rounded-2xl border-gray-200 bg-slate-900 text-emerald-300 p-4"></textarea>
