<div x-data="{ detik: 0, berjalan: true }"
    ...
    @if(!$sudahDikirim)
    <p class="text-sm font-semibold text-gray-600 mb-1">Tentukan status kehadiran 12 siswa berikut berdasarkan jam
        kedatangannya.</p>
    ...
    <table class="w-full text-sm border rounded-2xl overflow-hidden mb-4">
        <thead class="bg-teal-50 text-left">
            <tr>
                <th class="p-2 font-black">No</th>
                <th class="p-2 font-black">Nama</th>
                <th class="p-2 font-black">Jam</th>
                <th class="p-2 font-black">Status</th>
            </tr>
        </thead>
        <tbody>
            @foreach($data as $d)
    ...
    <div class="bg-teal-50 rounded-2xl p-5 text-sm font-semibold text-gray-700 leading-relaxed">
        <p>Kamu butuh <strong>{{ $waktuDetik }} detik</strong> untuk 12 siswa. Kekeliruanmu:
            <strong>{{ $this->jumlahKeliru }}</strong> dari 12.
        </p>
        <p class="mt-2">Sekolah ini punya 864 siswa. Kalau kecepatanmu tetap, satu hari rekap memakan waktu
            <strong>{{ $this->ekstrapolasi }}</strong>. Dan itu baru satu hari.
        </p>
        @if($this->keliruDiJamBatas)<p class="mt-2 text-amber-600">Kekeliruanmu ada di jam 07:00, 07:01, atau 07:30 —
            tepat di batas antar-status.</p>@endif
    </div>
